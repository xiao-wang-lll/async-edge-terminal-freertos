...
*偷瞄（r）tick
*/
// 假设 FreeRTOS 已经运行，SysTick 已经在干活了
// 且 SystemCoreClock 准确
void delay_us_systick(uint32_t us) {
    uint32_t ticks;
    uint32_t told, tnow, tcnt = 0;
    uint32_t reload = SysTick->LOAD; // 获取 FreeRTOS 设置的重装载值

    ticks = us * (SystemCoreClock / 1000000); // 需要多少个时钟周期
    told = SysTick->VAL; // 偷看一眼当前计数值

    while (1) {
        tnow = SysTick->VAL;
        if (tnow != told) {
            // SysTick 是倒着数的 (Down Counter)
            if (tnow < told)
                tcnt += told - tnow;
            else 
                tcnt += reload - tnow + told; // 发生了溢出重装载
