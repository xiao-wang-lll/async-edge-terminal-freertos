...
void main_page_display(void){
		...
		do{
			st7789_fill_color(15, 15, 225 - 1, 155 - 1, color_bg_time);
			st7789_write_image(23,20,&image_icon_wifi);
			main_page_redraw_wifi_ssid(WIFI_SSID);
			
			//(240-((76*5)/2))/2=25---时间
			st7789_write_string(25,37,"--:--",mkcolor(0, 0, 0),color_bg_time,&font76_mapple_extrabold);
			st7789_write_string(207,96,"--",mkcolor(0, 0, 0),color_bg_time,&font16_mapple);
			
			//(240-((20*17)/2))/2=---日期
			st7789_write_string(35,121,"----/--/-- 星期-",mkcolor(143, 143, 143),color_bg_time,&font20_mapple_bold);
		...
		do{
			st7789_fill_color(15, 165, 115 - 1, 305 - 1, mkcolor(136,217,234));
		
			st7789_write_string(19,170,"室内环境",mkcolor(0, 0, 0),color_bg_inner,&font20_mapple_bold);
			
			st7789_write_string(83,191,"℃",mkcolor(0, 0, 0),color_bg_inner,&font32_mapple_bold);
			st7789_write_string(91,262,"%",mkcolor(0, 0, 0),color_bg_inner,&font32_mapple_bold);
		...
		do{
			st7789_fill_color(125, 165, 225 - 1, 305 - 1, color_bg_outdoor);
			st7789_write_image(139,239,&image_icon_wenduji);
			main_page_redraw_outdoor_weather_icon(0);	
			
			st7789_write_string(127,170,"吕梁",mkcolor(0, 0, 0),color_bg_outdoor,&font20_mapple_bold);
			
			st7789_write_string(192,190,"℃",mkcolor(0, 0, 0),color_bg_outdoor,&font32_mapple_bold);
...
void main_page_redraw_wifi_ssid(const char *ssid){
			...
			st7789_write_string(50,23 ,str,mkcolor(143, 143, 143),color_bg_time,&font16_mapple);
...
void main_page_redraw_time(rtc_date_time_t *time){
		...
		st7789_write_string(25,37,str,mkcolor(0, 0, 0),color_bg_time,&font76_mapple_extrabold);
		
		char str_sec[3]={'-','-'};
		snprintf(str_sec,sizeof(str_sec),"%02u",time->second);
		st7789_write_string(207,96,str_sec,mkcolor(0, 0, 0),color_bg_time,&font16_mapple);
...
void main_page_redraw_date(rtc_date_time_t *date){
		...
		st7789_write_string(35,121,str,mkcolor(143, 143, 143),color_bg_time,&font20_mapple_bold);
...
void main_page_redraw_inner_temperature(float temperature){
		...
		st7789_write_string(30,192,str,mkcolor(0, 0, 0),color_bg_inner,&font54_mapple_bold);
...
void main_page_redraw_inner_humidity(float humidity){
		...
		st7789_write_string(25,239,str,mkcolor(0, 0, 0),color_bg_inner,&font64_mapple_extrabold);
...
void main_page_redraw_outdoor_city(const char *city){
		...
		st7789_write_string(127,170,str,mkcolor(0, 0, 0),color_bg_outdoor,&font20_mapple_bold);
...
void main_page_redraw_outdoor_temperature(float temperature){
	...
	char str[4]={'-','-'};
		...
		st7789_write_string(140,189 ,str,mkcolor(0, 0, 0),color_bg_outdoor,&font54_mapple_bold);
...
void main_page_redraw_outdoor_weather_icon(const int code){
		...
		st7789_write_image(166,240,icon);
}
